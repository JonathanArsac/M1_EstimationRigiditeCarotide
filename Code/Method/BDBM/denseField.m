function [chp_dep_int,pts_ax,pts_lat] = denseField(varargin)
...
% varargin{5}=type_interp: 1 = moyenne; 2 = m�diane
...
%points de calcul axial et lat�ral
...
%     timerbar((j-1)/Npt,progress_bar);
    ...
    for i=1:Mpt
        ...
        dep_lat=0;
        nb=1;
        %on considre les quatre points P (ceux ou on connait la valeur du champ estim�) autour du point consid�re
        for m=1:2
            for n=1:2
                %pour chacun des points P
                %calcul de la distance (x,y) au point consid�r�
                y=pts_ax(i)-varargin{3}(gr_ax(i,m));
                x=pts_lat(j)-varargin{2}(gr_lat(j,n));

                if((x<=varargin{4}(2)/2)&(y<=varargin{4}(1)/2))
                    %Calcul de la valeur du valeur du champ interpol� grace aux formules du d�placement bilin�aire
                    ...
                    v=x*varargin{1}(gr_ax(i,m),gr_lat(j,n),1,2)+y*varargin{1}(gr_ax(i,m),gr_lat(j,n),2,2)+...
                        x*y*varargin{1}(gr_ax(i,m),gr_lat(j,n),3,2)+varargin{1}(gr_ax(i,m),gr_lat(j,n),4,2);
        ...
        switch(varargin{5})
            %calcul du champ interpol� moyen au point consid�r�
            case 1
                %en prenant la moyenne apr�s avoir �liminer les valeurs qui
...
% close(progress_bar);
%interpolation � tous les points de l'image
...
u=chp_dep_int_p(:,:,1);
v=chp_dep_int_p(:,:,2);
%interpolation des composantes lat�rale et axiale
