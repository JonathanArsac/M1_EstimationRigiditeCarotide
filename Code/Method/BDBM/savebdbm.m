function [champ_dep_est, x_grid, y_grid]=bdbm(varargin);
...
B1 = varargin{1}(P_dep(1)-ceil(varargin{3}(1)/2)+1:P_dep(1)-ceil(varargin{3}(1)/2)+varargin{3}(1),...
    P_dep(2)-ceil(varargin{3}(2)/2)+1:P_dep(2)-ceil(varargin{3}(2)/2)+varargin{3}(2),1);
ZRtest = varargin{1}(P_dep(1)-ceil(varargin{3}(1)/2)+1-varargin{9}(1,1):P_dep(1)-ceil(varargin{3}(1)/2)+varargin{3}(1)+varargin{9}(1,2),...
    P_dep(2)-ceil(varargin{3}(2)/2)+1-varargin{9}(2,1):P_dep(2)-ceil(varargin{3}(2)/2)+varargin{3}(2)+varargin{9}(2,2),2);

[pos_best_corr, vcorr]=CostFunctions(B1,ZRtest,[1 1],1);

varargin{9}=(pos_best_corr-1-varargin{9}(:,1)');
...
for cercle_it=0:nb_boucle
    ...
    for jN=borne_min_N:borne_max_N
        ...
        for n = 1:(N-1) %images loop
            ...
            if n==1 %estimation between the first two images
            ...
            else %cas à partir de la 2ème et la 3ème image

                %prendre des précautions pour l'initialisation quand n > 1
                ...
                if jM==0 & jN==0
                    ...
                    if jN~=0
                        seuil = inf;
                        if abs(varargin{4}(1)*jM + sum(champ_dep_est(jM+1,N_mat_dep+jN,4,1,:)) - (varargin{4}(1)*jM + sum(champ_dep_est(jM+1,N_mat_dep+jN-sign(jN),4,1,:)))) < seuil

                            dP(1)=round(champ_dep_est(jM+1,N_mat_dep+jN-sign(jN),4,1,n));

                        else

                            %initialiser dans le cas où les points d'initialisation sont trop décalés
                    ...
                    if jM~=0
                        ...
                        seuil = inf;
                        if abs(varargin{4}(2)*jN + sum(champ_dep_est(jM+1,N_mat_dep+jN,4,2,:)) - (varargin{4}(2)*jN + sum(champ_dep_est(jM+1,N_mat_dep+jN-sign(jN),4,2,:)))) < seuil
            ...
            if n == 1
                ...
                P = P_dep+varargin{4}.*[jM jN] + [sum(champ_dep_est(jM+1,N_mat_dep+jN,4,1,:)) sum(champ_dep_est(jM+1,N_mat_dep+jN,4,2,:))];
                Pat_ini = newgrille(varargin{1}(:,:,n),P,varargin{3},varargin{6},n,varargin{5});
                P = round(P);
            end

            %Current study zone on the first image
            if n==1
                Pat_ini=varargin{1}(P(1)-varargin{3}(1):P(1)+varargin{3}(1),P(2)-varargin{3}(2):P(2)+varargin{3}(2),n);
            else
                Pat_ini=Pat_ini;
            end

            if varargin{7} == 5 % LSP estimator on the correlation function
                ZR = varargin{1}(P(1)-varargin{3}(1)+dP(1):P(1)+varargin{3}(1)+dP(1),P(2)-varargin{3}(2)+dP(2):P(2)+varargin{3}(2)+dP(2),n+1);
            ...
            elseif varargin{7} == 6 % LSP estimator directly on the signals
                ...
                ZR=varargin{1}(PsZ(1):PsZ(1)+tROI(1),PsZ(2):PsZ(2)+tROI(2),n+1);  %Search zone on the second image
                %estimation du champ de déplacement en P
                dep=bilinearEstimation(Pat_ini,ZR,varargin{3},varargin{10},varargin{6},varargin{5},varargin{7},varargin{8},varargin{11});
            end
