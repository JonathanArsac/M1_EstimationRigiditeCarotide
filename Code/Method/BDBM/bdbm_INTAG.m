function [champ_dep_est, x_grid, y_grid]=bdbm(im,Z_im,L,Grid,vI,nb_it,t_cor,type_calcul_dep,dP_ini_limittR,f,US_IRM);
    ...
    for jN=borne_min_N:borne_max_N
        ...
        for n = 1:(N-1) %images loop
            ...
            if t_cor == 6 % LSP estimator directly on the signals
                ...
                %estimation du champ de déplacement en P
                dep=bilinearEstimation(Pat_ini,ZR,L,tR,nb_it,vI,t_cor,type_calcul_dep,f);
            end
