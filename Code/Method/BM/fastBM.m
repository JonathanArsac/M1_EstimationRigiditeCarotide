function [champ_dep_est, x_grid, y_grid]=fastBM(im1,im2,Z_im,L,Grid,vI,type_correl,dP_ini_limit,tR,f);
...
elseif type_correl == 6
    ...
    delta1 = floor(1/(4*f(1))); delta2 = floor(1/(4*f(2))); %delta2=2;

    % sin cos
    im1_QI = im1((delta1+1):end,1:(end-delta2));
    % cos sin
    im1_IQ = im1(1:(end-delta1),(delta2+1):end);
    % sin sin
    im1_QQ = im1((delta1+1):end,(delta2+1):end);
    im1 = im1(1:(end-delta1),1:(end-delta2));

    % sin cos
    im2_QI = im2((delta1+1):end,1:(end-delta2));
    % cos sin
    im2_IQ = im2(1:(end-delta1),(delta2+1):end);
    % sin sin
    im2_QQ = im2((delta1+1):end,(delta2+1):end);
    im2 = im2(1:(end-delta1),1:(end-delta2));
     
    P1_im1=-atan2(im1_IQ+im1_QI,im1-im1_QQ);
    P2_im1=-atan2(im1_IQ-im1_QI,im1+im1_QQ);
    P1_im2=-atan2(im2_IQ+im2_QI,im2-im2_QQ);
    P2_im2=-atan2(im2_IQ-im2_QI,im2+im2_QQ);
...
for cercle_it=0:nb_boucle
    ...
    for jN=borne_min_N:borne_max_N
        ...
        elseif type_correl==6
            ZR = im2(P(1)-ceil(L(1)/2)+1+dP(1):P(1)-ceil(L(1)/2)+L(1)+dP(1),P(2)-ceil(L(2)/2)+1+dP(2):P(2)-ceil(L(2)/2)+L(2)+dP(2));
            [dPy dPx] = PhaseAdjust3(P1_im1(P(1)-ceil(L(1)/2)+1:P(1)-ceil(L(1)/2)+L(1),P(2)-ceil(L(2)/2)+1:P(2)-ceil(L(2)/2)+L(2)),...
