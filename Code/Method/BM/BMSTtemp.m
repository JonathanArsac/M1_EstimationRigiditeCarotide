function [champ_dep_est, x_grid, y_grid]=BMSTtemp(im1,im2,dep_init,Z_im,L,Grid,vI,type_correl,tR,f,US_IRM);

%2-D motion estimation with block matching (speckle tracking)
%designed for Lagrangian estimation (reported to a fix reference I1)
...
%  im1 :   1st image
%  im2 :   2nd image
%  dep_init : nodes displacements for images I1 In-1
...
%  US_IRM : 1 if RF, 0 if tagged RMI
%OUTPUT
...
for cercle_it=0:nb_boucle
    ...
    for jN=borne_min_N:borne_max_N
        ...
        if type_correl==6
            if US_IRM==0 %%%centrer les signaux autour de 0
                ZR = im2(P(1)-ceil(L(1)/2)+1+dP(1):P(1)-ceil(L(1)/2)+L(1)+dP(1),P(2)-ceil(L(2)/2)+1+dP(2):P(2)-ceil(L(2)/2)+L(2)+dP(2));
                [dPx dPy] = PhaseAdjust2_IRM(im1(P(1)-ceil(L(1)/2)+1:P(1)-ceil(L(1)/2)+L(1),P(2)-ceil(L(2)/2)+1:P(2)-ceil(L(2)/2)+L(2))...
