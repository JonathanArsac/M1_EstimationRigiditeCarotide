function [champ_dep_est, x_grid, y_grid]=BMP(im1,im2,Z_im,L,Grid,vI,type_cor1,type_cor2,tR,f,type_fenetre)
...
if type_cor2 == 5
    ...
    tR = [1 1 ; 1 1];
elseif (type_cor2 == 6)||(type_cor2 == 9)||(type_cor2 == 12)
    vI = [1 1]; % no interpolation in the case of LSE phase estimator
...
if (type_cor2 == 9)||(type_cor2 == 12)
champ_dep_est=NaN*ones(M_max+1,N_max,3,2);
else
clear type_fenetre
...
%         dep_init2(jM+1,jN+1,:)=dep_init;      
        ...
        if max(max(abs(Pat_ini)))==0
            ...
            elseif type_cor2==6
             ...
             elseif type_cor2==9 %%estimation locale des parametre du modele affine
                [U V] = PhaseAdjustAffine(P1_im1(P(1)-ceil(L(1)/2)+1:P(1)-ceil(L(1)/2)+L(1),P(2)-ceil(L(2)/2)+1:P(2)-ceil(L(2)/2)+L(2)),...
                    P2_im1(P(1)-ceil(L(1)/2)+1:P(1)-ceil(L(1)/2)+L(1),P(2)-ceil(L(2)/2)+1:P(2)-ceil(L(2)/2)+L(2))...
                    ,P1_im2(P(1)-ceil(L(1)/2)+1+dP(1):P(1)-ceil(L(1)/2)+L(1)+dP(1),P(2)-ceil(L(2)/2)+1+dP(2):P(2)-ceil(L(2)/2)+L(2)+dP(2))...
                    ,P2_im2(P(1)-ceil(L(1)/2)+1+dP(1):P(1)-ceil(L(1)/2)+L(1)+dP(1),P(2)-ceil(L(2)/2)+1+dP(2):P(2)-ceil(L(2)/2)+L(2)+dP(2)),f(2),f(1),type_fenetre);
 
                V(1)=V(1)+dP(1);
                U(1)=U(1)+dP(2);

             elseif type_cor2==12 %%estimation locale des parametre du modele affine

                [U V] = PhasePlanAdjustAffine(P1_im1(P(1)-ceil(L(1)/2)+1:P(1)-ceil(L(1)/2)+L(1),P(2)-ceil(L(2)/2)+1:P(2)-ceil(L(2)/2)+L(2)),...
                    P2_im1(P(1)-ceil(L(1)/2)+1:P(1)-ceil(L(1)/2)+L(1),P(2)-ceil(L(2)/2)+1:P(2)-ceil(L(2)/2)+L(2))...
                    ,P1_im2(P(1)-ceil(L(1)/2)+1+dP(1):P(1)-ceil(L(1)/2)+L(1)+dP(1),P(2)-ceil(L(2)/2)+1+dP(2):P(2)-ceil(L(2)/2)+L(2)+dP(2))...
                    ,P2_im2(P(1)-ceil(L(1)/2)+1+dP(1):P(1)-ceil(L(1)/2)+L(1)+dP(1),P(2)-ceil(L(2)/2)+1+dP(2):P(2)-ceil(L(2)/2)+L(2)+dP(2)),f(2),f(1),type_fenetre);
...
% figure;subplot(211);imagesc(dep_init2(:,:,1));colorbar
% subplot(212);imagesc(dep_init2(:,:,2));colorbar
