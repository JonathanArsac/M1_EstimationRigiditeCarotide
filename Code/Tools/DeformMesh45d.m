function H=DeformMesh45d(im,patchSize,x,y,dep_ax,dep_lat,x_grid,y_grid,L)
...
for k = 1:(N-1)
    ...
    for B1=2:2:(size(x,2)-2) % pour chaque bloc affiché
        for B2=2:2:(size(y,2)-2)
            line([x(B1)+d1(k,B1,B2) x(B1)+patchSize(1)+d1(k,B1+1,B2+1)], [y(B2)+d2(k,B1,B2) y(B2)+patchSize(2)+d1(k,B1+1,B2+1)],'Color','w','LineWidth',0.5)          
            line([x(B1)+d1(k,B1,B2) x(B1)+patchSize(1)+d1(k,B1+1,B2-1)], [y(B2)+d2(k,B1,B2) y(B2)-patchSize(2)+d1(k,B1+1,B2-1)],'Color','w','LineWidth',0.5)          
            line([x(B1)+d1(k,B1,B2) x(B1)-patchSize(1)+d1(k,B1-1,B2-1)], [y(B2)+d2(k,B1,B2) y(B2)-patchSize(2)+d1(k,B1-1,B2-1)],'Color','w','LineWidth',0.5)          
            line([x(B1)+d1(k,B1,B2) x(B1)-patchSize(1)+d1(k,B1-1,B2+1)], [y(B2)+d2(k,B1,B2) y(B2)+patchSize(2)+d1(k,B1-1,B2+1)],'Color','w','LineWidth',0.5)          
        end
